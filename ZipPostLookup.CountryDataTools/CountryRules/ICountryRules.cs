...
public interface ICountryRules
{
    ...
    /// <summary>
    /// True when the country's rules provide a deterministic Admin1 derivation
    /// via <see cref="ResolveAdmin1"/>. Replaces the old reflection-based sniff
    /// (<c>GetMethod("ResolveAdmin1").DeclaringType != typeof(ICountryRules)</c>).
    /// Default: false (interface default implementation returns null from ResolveAdmin1).
    /// </summary>
    bool SupportsAdmin1Derivation => false;
    ...
    /// <summary>
    /// Languages used by <see cref="Validation.PlaceNameNormalizer"/> when expanding
